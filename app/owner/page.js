...
export default function OwnerInbox() {
  ...
  async function load(accessToken) {
    ...
    if (res.ok) {
      ...
      setMessage('Unable to load requests. This account may not have staff access.');
    }
    setLoading(false);
  }
  ...
  async function login(e) {
    ...
    if (!res.ok || !data.access_token) {
      setMessage('Sign-in failed. Check your email and password.'); setLoading(false); return;
    }
  ...
  async function setStatus(id, status) {
    const res = await sb(`/rest/v1/public_service_requests_v1?id=eq.${encodeURIComponent(id)}`, {
