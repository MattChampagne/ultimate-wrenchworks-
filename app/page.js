...
const services = [
  ['SXS / UTV', 'sxs'],
  ['ATV', 'atv'],
  ['Motorcycle / Dirt Bike', 'motorcycle'],
  ['Auto / Light Truck', 'auto']
];

function ServiceRender({ type }) {
  return <span className={`serviceRender serviceRender-${type}`} aria-hidden="true" />;
}
...
export default function Home() {
  return (
    <main className="customerSite">
      <nav className="nav customerNav"><a className="brand customerBrand" href="#top"><span>UW</span><b>ULTIMATE<br/>WRENCHWORKS</b></a><div className="navlinks"><a href="#services">Services</a><a href="#process">How It Works</a><a className="navcta" href="#schedule">Schedule Service</a></div></nav>
      ...
      <section className="section serviceShowcase" id="services"><div className="sectionHeading"><p className="kicker">WHAT WE WORK ON</p><h2>POWERSPORTS TO DAILY DRIVERS.</h2><p>Mobile service designed to keep your machines moving without the hassle of hauling them to a shop.</p></div><div className="grid serviceGrid serviceImageGrid">{services.map(([title, type])=><article className="serviceImageTile" key={title}><ServiceRender type={type} /><h3>{title}</h3></article>)}</div></section>
